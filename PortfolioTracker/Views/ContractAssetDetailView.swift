...
struct ContractAssetDetailView: View {
    ...
    private var categoryTotalValue: Double {
        guard let cat = asset.category else { return totalBalance }
        ...
        return categoryAssets.reduce(0.0) { sum, a in
            sum + (PortfolioMetrics.currentValue(for: a) * activeRate)
        }
    }
    ...
    private var categoryAllocationPercentage: Double {
        categoryTotalValue > 0 ? (totalBalance / categoryTotalValue) * 100.0 : 0.0
    }
    
    private var netWorthAllocationPercentage: Double {
        portfolioTotalNetWorth > 0 ? (totalBalance / portfolioTotalNetWorth) * 100.0 : 0.0
    }
